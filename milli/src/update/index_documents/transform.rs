...
impl<'a, 'i> Transform<'a, 'i> {
        ...
        FA: Fn() -> bool + Sync,
    {
        ...
        while let Some(enriched_document) = cursor.next_enriched_document()? {
            // tracing::debug!("Processing document number: {}", documents_count + 1);

            let EnrichedDocument { document, document_id } = enriched_document;

            if should_abort() {
                tracing::warn!("Aborting indexation due to should_abort condition");
            ...
            if document_id.is_generated() {
                ...
                // tracing::debug!("Generated document ID added: {:?}", docid_buffer);
            }
                    ...
                    let docid = match external_documents_ids.get(wtxn, entry.key())? {
                        Some(docid) => {
                            if self.replaced_documents_ids.insert(docid) {
                                ...
                                // tracing::debug!("Replaced document ID: {}", docid);
                            }
                            docid
                        }
                        None => self
            ...
            // tracing::debug!("Document ID after mapping: {}", docid);
            ...
            if let Some(original_docid) = original_docid {
                ...
                if base_obkv == obkv_buffer {
                    ...
                    skip_insertion = true;
                    // tracing::debug!("Skipping insertion, document already exists");
                } else {
                    // tracing::debug!("Document needs updating, preparing for replacement");
            ...
            if !skip_insertion {
                ...
                self.original_sorter
                    .insert(&document_sorter_key_buffer, &document_sorter_value_buffer)?;
                // tracing::debug!("Document added to sorter: {}", docid);

                // Additional logs after adding the document to the sorter
                // tracing::debug!("Document sorter key buffer: {:?}", document_sorter_key_buffer);
                // tracing::debug!("Document sorter value buffer: {:?}", document_sorter_value_buffer);
                    ...
                    Self::flatten_from_fields_ids_map(&flattened_obkv, &mut self.fields_ids_map)?
                {
                    ...
                    // tracing::debug!("Flattened document added to sorter: {}", docid);
                    // tracing::debug!("Flattened OBKV buffer: {:?}", obkv);
                } else {
                    tracing::debug!("No flattened document created for docid: {}", docid);
                }
            } else {
                tracing::debug!("Skipping insertion for docid: {}", docid);
            }
        ...
        tracing::debug!("Finished processing documents, total count: {}", documents_count);
    ...
    fn flatten_from_fields_ids_map(
        ...
        tracing::debug!("Entering flatten_from_fields_ids_map function");

        // Print fields_ids_map
        // for (key, value) in fields_ids_map.iter() {
        //     tracing::debug!("Field ID: {}, Field Name: {}", key, value);
        // }

        let initial_field_count = fields_ids_map.len();
        tracing::debug!("Initial field count: {}", initial_field_count);

        // Skip flattening if none of the fields need it
        if obkv
            .iter()
            .all(|(_, value)| !json_depth_checker::should_flatten_from_unchecked_slice(value))
        {
            tracing::debug!("No fields need to be flattened, returning None");
            return Ok(None);
        }

        tracing::debug!("Flattening fields from obkv");
        ...
        for (key, value) in obkv.iter() {
            ...
            tracing::debug!("Checking field for flattening: {}", key_name);
            ...
            // If it should be flattened, add it to the document
            if json_depth_checker::should_flatten_from_unchecked_slice(value) {
                tracing::debug!("Flattening field: {}", key_name);
                let value = serde_json::from_slice::<Value>(value)
                    .map_err(crate::error::InternalError::SerdeJson)?;
                doc.insert(key_name.to_string(), value);
            } else {
                key_value.push((key, value.into()));
            }
        }
        ...
        // tracing::debug!("Flattened JSON: {:?}", flattened);

        let mut added_fields_count = 0;

        for (key, value) in flattened.into_iter() {
            tracing::debug!("Inserting flattened field: {}", key);
            match fields_ids_map.insert(&key) {
                Some(fid) => {
                    // tracing::debug!("Field ID for {}: {}", key, fid);
                ...
                None => {
                    tracing::error!(
                        "Attribute limit reached when inserting field: {}. Total fields: {}",
                        key,
                        initial_field_count + added_fields_count
                    );
                    return Err(Error::UserError(UserError::AttributeLimitReached));
                }
            }
        }

        let total_field_count = initial_field_count + added_fields_count;
        tracing::debug!("Total fields after flattening: {}", total_field_count);

        key_value.sort_unstable_by_key(|(key, _)| *key);

        let mut buffer = Vec::new();
        Self::create_obkv_from_key_value(&mut key_value, &mut buffer)?;
        tracing::debug!("Created obkv from key_value");

        tracing::debug!("Exiting flatten_from_fields_ids_map function");
        Ok(Some(buffer))
    }
